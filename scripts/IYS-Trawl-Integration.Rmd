...
IYS_events <- IYS_events %>%
  mutate(dataset_doi <- case_when(
    vessel_name_abbr == "TINRO" ~ "https://doi.org/10.21966/wevm-ww19",
    vessel_name_abbr == "Shimada" ~ "https://doi.org/10.21966/nt8w-je90",
    vessel_name_abbr == "Franklin" ~ "https://doi.org/10.21966/gmrz-ad56", # DOI minted, metadata record not yet 'live'. 
    vessel_name_abbr == "NW" ~ "https://doi.org/10.21966/shnm-s480",
    vessel_name_abbr == "Kaganovsky" ~ "https://doi.org/10.21966/DPHZ-AS73",
    vessel_name_abbr == "Legacy" ~ "https://doi.org/10.21966/4J6T-GB64"))
...
IYS_trawl_catch <- bind_rows(TINRO_catch, NW_catch, Shimada_catch, Franklin_catch)
...
IYS_trawl_catch$verbatim_identification <- IYS_trawl_catch$scientific_name
...
IYS_scientificnames <- worrms::wm_records_names(unique(IYS_trawl_catch$scientific_name)) %>% bind_rows() %>% rename(scientific_name = scientificname) %>%
...
# We see that in the overall catch data there are 9 (including 'NA') species that don't have a match in the WoRMS Registry, change these names in the original dataframe:
...
IYS_trawl_catch$scientific_name <- gsub("^Amphipod$", "Amphipoda", IYS_trawl_catch$scientific_name) # Please not the ^ and $ to indicate start and 
...
#TODO calculate sampling_depth_meters for TINRO based on pressure_dbar whatever else TEOS tells me
