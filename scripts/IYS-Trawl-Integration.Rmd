...
IYS_events <- IYS_events %>%
  mutate(dataset_doi <- case_when(
    ...
    vessel_name_abbr == "Franklin" ~ "https://doi.org/10.21966/gmrz-ad56", 
...
```{r match 2019 ctd to trawl}
# Isolate 2019 CTD events from IYS_events
nt_ctd_e <- IYS_events |> 
  filter(vessel_name_abbr == "Kaganovsky",
         event_type == "CTD") |> 
  select(station, station_event_id, event_date)

#Isolate 2019 CTD events from IYS_CTD data
df2 <- IYS_CTD |> 
  select(station_event_id, event_date)

#join CTD data to CTD events
df3 <- semi_join(nt_ctd_e, df2, by = "station_event_id")

#That works as expected and gets the station number onto the CTD 

nt_trawl_e <- IYS_events |> 
  filter(vessel_name_abbr == "Kaganovsky",
         event_type == "Trawl") |> 
  select(station, station_event_id, event_date)

join <- full_join(nt_trawl_e, nt_ctd_e, by = "station")
join
```
