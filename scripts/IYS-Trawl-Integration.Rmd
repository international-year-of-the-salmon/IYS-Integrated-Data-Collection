...
download.file("https://github.com/international-year-of-the-salmon/2022-shimada-data-template/blob/main/original_data/Shimada%20adhesion%20scar%20data.xlsx?raw=true", here::here("input_datasets", "IYS2022_Shimada_MarksScars_Data.xlsx"), quiet = TRUE, mode = "wb")
...
```{r event data}
# For the Trawl data:
NW_event <- read_excel(here("input_datasets", "IYS2022_NW_Data.xlsx"), sheet = "Sampling_Event_Info") %>%
...
# Add Bongo data from 2019 and 2020. Make sure you first - manually - add the latest version of the data to the folder. The GitHub repos are currently private, so the files are manually added to the 'input_datasets' folder. Select only data for net 1, which was used for taxonomic analysis.
Bongo2019_event <- read.csv(here("input_datasets", "bongo2019_event.csv"), stringsAsFactors = F)
Bongo2019_event <- Bongo2019_event[63:178,] 
Bongo2019_event <- Bongo2019_event %>% 
  filter(grepl("Net1", eventID))

# Add bottom depth:
Bongo2019_bottomdepth <- read_excel(here("input_datasets", "IYS cruise logbook_2019.xlsx"), 
...
Bongo2020_event <- read.csv(here("input_datasets", "bongo2020_event.csv"), stringsAsFactors = F)
...
# Trawl data was recorded in Z+1200 - change to UTC:
...
janitor::compare_df_cols(NW_event, TINRO_event, Shimada_event, Franklin_event, 
                         raw_spirit_event, bongo_event, GoA2019_event, GoA2020_event)

# Join the individual event data frames from all IYS High Seas Expeditions and make all column names :
IYS_events <- bind_rows(TINRO_event, NW_event, Shimada_event, Franklin_event, bongo_event, raw_spirit_event, GoA2019_event, GoA2020_event) |> 
...
```{r IYS Catch clean}
# For TINRO catch:
TINRO_catch <- read_excel(here("input_datasets", "IYS2022_TINRO_Data.xlsx"), 
...
NW_catch <- read_excel(here("input_datasets", "IYS2022_NW_Data.xlsx"), 
                       sheet = "Catch_Info")
...
Shimada_catch <- read_excel(here("input_datasets", "IYS2022_Shimada_Data.xlsx"), 
                            sheet = "5. CATCH_FINAL INFO")
...
# For Franklin catch:
Franklin_catch <- read_excel(here("input_datasets", "IYS2022_Franklin_Data.xlsx"), 
...
GoA2020_catch <- read_excel(here("input_datasets", "2020_GoA_Fish_Trawl_data.xlsx"), 
                            sheet = "CATCH_FINAL")
...
# Join station_event_ids from event table to catch_ids, to see if there are any recorded sampling events that didn't end up catching anything (i.e. have NA in the catch_id):
trawl_event <- IYS_events %>% filter(event_type == "Trawl") %>% select(station_event_id)
IYS_trawl_catch <- left_join(trawl_event, IYS_trawl_catch, by = "station_event_id") 
catch_id_null <- IYS_trawl_catch[is.na(IYS_trawl_catch$catch_id),]
...
IYS_trawl_catch$scientific_name <- gsub("Hormiphora cucumisÂ", "Hormiphora cucumis", IYS_trawl_catch$scientific_name)
...
Shimada_scars <- read_excel(here("input_datasets", "IYS2022_Shimada_MarksScars_Data.xlsx"), sheet = "Sheet1")
...
GoA2019_specimen_tab <- GoA2019_specimen_tab %>%
  mutate(lifestage = ifelse(sex == "j", "juvenile", lifestage),
         ...
         lifestage = ifelse(sex == "1.0", "immature", lifestage))
...
#TODO: confirm 1.0 is indeed immature.
...
GoA2020_specimen <- read_excel(here("input_datasets", 
...
IYS_trawl_specimen$scientific_name <- gsub("Hormiphora cucumisÂ", "Hormiphora cucumis", IYS_trawl_specimen$scientific_name)
...
IYS_trawl_specimen <- left_join(IYS_trawl_specimen, IYS_spp_scientificnames, by = "scientific_name") %>%
  select(-c(AphiaID, alternative_id, stomach_fullness, gonad_weight_grams, `weight_units...11`,
            ...
            `weight_units...20`, species_updated, scalecard_position, scale_pref_code, institutioncode,
...
Next, add the Bongo Specimen data (please note, we do not currently have the Franklin Bongo data):

# Zooplankton

```{r Zooplankton}
zoop_2019_2020 <- read_excel(here("input_datasets", "IYS Zooplankton Taxonomy Abundance Data.xlsx"),
                        sheet = "All data", 
                        col_types = c("text", "date", "date", "numeric", "numeric", "numeric", "text",
...
zoop_2019_2020 <- zoop_2019_2020 %>%
  dplyr::select(station,
         ...
         abundance_m3 = abund_m3_flow_est,
...
zoop_2019_2020 <- zoop_2019_2020 %>%
  mutate(net_number = "1",
         station_event_id = ifelse(grepl("2019", date), 
                                   paste("IYS2019", "Kaganovsky", station, "Bongo", net_number, sep = "-"),
                                   paste("IYS2020", "Legacy", station, "Bongo", net_number, sep = "-"))) %>%
  group_by(station_event_id) %>%
  mutate(catch_id = paste(station_event_id, row_number(), sep = "-"),
         species_recorded = NA,
...
dups <- zoop_2019_2020 %>% group_by(catch_id) %>% filter(n() >1)

zoop_2022 <- read_excel(here("input_datasets", "IYS2022_Zooplankton_Template.xlsx"), 
                   sheet = "Bongo_Specimen_Data")

zoop_2022 <- sapply(zoop_2022, as.character)
zoop_2022[is.na(zoop_2022)] <- ""
zoop_2022 <- as_tibble(zoop_2022)
zoop_2022$verbatim_identification <- zoop_2022$scientific_name
...
# Reshuffle columns:
zoop <- zoop %>% 
  dplyr::select(station_event_id, catch_id, scientific_name, verbatim_identification, 
                lifestage, sex, catch_count, everything())

# Standardize the lifestage column:
zoop <- zoop %>%
  mutate(lifestage = gsub("calyptopsis|^cal$|caliptopis", "calyptopis", lifestage),
         lifestage = gsub("furcillia", "furcilia", lifestage),
         lifestage = gsub("^I$", "C1", lifestage),
         lifestage = gsub("^II$", "C2", lifestage),
         lifestage = gsub("^III$", "C3", lifestage),
         lifestage = gsub("^IV$", "C4", lifestage),
         lifestage = gsub("^V$", "C5", lifestage),
         lifestage = gsub("^IVV$", "C4/C5", lifestage),
         lifestage = gsub("C4C5", "C4/C5", lifestage),
         lifestage = gsub("napulius", "nauplius", lifestage))
...
# Where sex is juvenile, make sure this information is captured in the lifestage column:
zoop <- zoop %>%
  mutate(lifestage = ifelse(zoop$sex == "juvenile", "juvenile", lifestage))
zoop$sex <- gsub("juvenile", "", zoop$sex)

# Remove " sp." and special characters (i.e. brackets) from scientific_name and adjust spelling: 
zoop$scientific_name <- gsub("\\b sp.\\b", "", zoop$scientific_name)
...
species_recorded <- zoop %>% select(species_recorded, lifestage) %>% unique()
...
# WoRMS - quick workaround: wm_records_name throws an error when dealing with our unique species list (n=233) as it says it's too long. Breaking it up in 2 separate lists seems to make it work. Issue generated here: https://github.com/ropensci/worrms/issues/39
...
sci_names_1 <- sci_names[1:128,]
sci_names_2 <- sci_names[129:247,]
...
worms_id <- bind_rows(worms_id_1, worms_id_2)
...
zoop$scientific_name <- gsub("Chaetognatha sp. (no heads)", "Chaetognatha", zoop$scientific_name, fixed = TRUE)
...
zoop$scientific_name <- gsub("Scaphocalanus brevicaudatus", "Scaphocalanus brevicornis", zoop$scientific_name) #TODO: confirm
...
sci_names <- unique(zoop$scientific_name) %>% as.data.frame()
colnames(sci_names)[1] <- "scientificname"
sci_names_1 <- sci_names[1:116,]
sci_names_2 <- sci_names[117:232,]
...
worms_id <- worms_id %>% 
  filter(!AphiaID %in% c("426434", "719191", "605966", "138835", "503291", "163921", "835292")) %>%
  select(-AphiaID)
...
dups_zoop <- zoop %>% group_by(catch_id) %>% filter(n() > 1)

# Annoyingly, in the sf_proc column there are some values such as 5-10, that in Excel get read in as dates. To work around this:
zoop$sf_proc <- as.character(zoop$sf_proc)
zoop$sf_proc <- paste0(" ", zoop$sf_proc)
...
shimada_CTD <- read_excel(here("input_datasets", "IYS2022_Shimada_Data.xlsx"), sheet = "7. CTD INFO") |> 
...
```{r 2022 Franklin CTD}
franklin_CTD <- read_excel(here("input_datasets", "IYS2022_Franklin_Data.xlsx"), sheet = "6. CTD INFO") |> 
