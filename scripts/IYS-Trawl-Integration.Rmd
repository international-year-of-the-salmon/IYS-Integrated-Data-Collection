...
NW_event <- read_excel(here("input_datasets", "IYS2022_NW_Data.xlsx"), sheet = "Sampling_Event_Info") %>%
  filter(Event_Type %in% c("CanTrawl", "Bongo")) |> 
...
# Add Bongo data from 2019 and 2020. Make sure you first - manually - add the latest version of the data to the folder. The GitHub repos are currently private, so the files are manually added to the 'input_datasets' folder:
...
Clean up the Bongo2019, Bongo2020, and Bongo2022 Zooplankton data:
...
unid_species <- left_join(IYS_trawl_catch, IYS_scientificnames, by = "scientific_name") %>% filter(is.na(AphiaID)) 
unid_species <- unique(unid_species$scientific_name)

# We see that there are x species that don't have a match in the WoRMS Registry, change these names in the original dataframe (column: scientific_name):
...
IYS_trawl_catch$scientific_name <- gsub("^Amphipod$", "Amphipoda", IYS_trawl_catch$scientific_name) # Please note the ^ and $ to indicate start and 
...
IYS_trawl_catch$scientific_name <- gsub("Unknown fish", "Pisces", IYS_trawl_catch$scientific_name)
...
NW_specimen <- NW_specimen %>%
  dplyr::rename(Station_Event_ID = `Station Event ID`,
                ...
                original_specimen_id = `Alternate ID`) %>%
  rename_with(tolower) %>%
  mutate(verbatim_identification = scientific_name) 
...
# IYS_trawl_specimen$scientific_name <- gsub("Paralepidadae", "")
...
IYS_trawl_specimen <- left_join(IYS_trawl_specimen, IYS_scientificnames, by = "scientific_name") %>%
  rename(scientific_name_id = lsid) %>%
  select(-c(AphiaID, alternative_id, stomach_fullness, gonad_weight_grams, `marks/scars`, `visceral adhesion`, `weight_units...11`,
            `fat reading 1`, `fat reading 2`, `fat reading 3`, `fat reading units`, `gonad weight`, `weight_units...19`,
            species_updated))
...
```{r Zooplankton}
zoop_2019 <- read_excel(here("input_datasets", "IYS Zooplankton Taxonomy Abundance Data.xlsx"),
                        sheet = "All data")

zoop_2019 <- sapply(zoop_2019, as.character)
zoop_2019[is.na(zoop_2019)] <- ""
zoop_2019 <- as_tibble(zoop_2019)
...
zoop_2019 <- zoop_2019 %>%
  mutate(lifestage = ifelse(grepl("C1", zoop_2019$species), "C1", lifestage),
         lifestage = ifelse(grepl("C2", zoop_2019$species), "C2", lifestage),
         lifestage = ifelse(grepl("C3", zoop_2019$species), "C3", lifestage))

zoop_2019 <- zoop_2019 %>%
  mutate(species = gsub("C1|C2|C3", "", zoop_2019$species),
         scientific_name = paste(genus, species))

zoop_2019 <- zoop_2019 %>%
  mutate(net_number = "1",
         station_event_id = paste("IYS2019", "Kaganovsky", station, "Bongo", net_number, sep = "-")) %>%
  ...
  mutate(catch_id = paste(station_event_id, row_number(), sep = "-"),
         ...
         size_range = NA, 
         count_method = NA) %>%
  select(-c(station, genus, species))

zoop_2020 <- read_excel(here("input_datasets", "Bongo_taxa.xlsx"),
                        sheet = "All data", col_types = "text")

# Please note that I'm reading in all the columns as character strings, because otherwise R will assume station column is numeric, whereas some stations are strings and therefore the output for those is NA. This messes up the date/time columns, but we don't need these for the specimen data file anyway. 

zoop_2020[1] <- lapply(zoop_2020[1], function (x) {
  x1 <- type.convert(as.character(x), as.is = TRUE) 
  ifelse(grepl("^[0-9.]+$", x1), round(as.numeric(x1)), x1)
})

zoop_2020 <- sapply(zoop_2020, as.character)
zoop_2020[is.na(zoop_2020)] <- ""
zoop_2020 <- as_tibble(zoop_2020)

zoop_2020 <- zoop_2020 %>%
  mutate(net_number = "1",
         station_event_id = paste("IYS2020", "Legacy", station, "Bongo", net_number, sep = "-")) %>%
  group_by(station_event_id) %>%
  mutate(catch_id = paste(station_event_id, row_number(), sep = "-"),
         species_recorded = NA,
         preservation_method = "formalin", 
...
# Some entries in the species column depict a lifestage:
zoop_2020 <- zoop_2020 %>%
  mutate(lifestage = ifelse(zoop_2020$species == "C1", "C1", lifestage),
         lifestage = ifelse(zoop_2020$species == "C2", "C2", lifestage),
         lifestage = ifelse(zoop_2020$species == "C3", "C3", lifestage))

zoop_2020 <- zoop_2020 %>%
  mutate(species = gsub("C1|C2|C3", "", zoop_2020$species),
         scientific_name = paste(genus, species)) %>%
...
janitor::compare_df_cols(zoop_2019, zoop_2020, zoop_2022)

zoop <- rbind(zoop_2019, zoop_2020, zoop_2022)

# There are still some scientific_names that are slightly misspelled. In order to make sure that scientific names are interoperable across data received from the vessels, we change the scientific_names to reflect "approved" names under the WoRMS Registry. THe original recorded name will still be included under the verbatim_identification header.
zoop$scientific_name <- trimws(zoop$scientific_name, "r")
worms_id <- worrms::wm_records_names(unique(zoop$scientific_name), marine_only = F) %>% dplyr::bind_rows()
...
zoop <- write_csv(zoop, here("output_datasets", "IYS_Bongo_specimen.csv"))
```
