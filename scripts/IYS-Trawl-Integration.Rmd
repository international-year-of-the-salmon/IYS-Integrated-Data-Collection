...
Franklin_event <- Franklin_event %>%
  ...
  select(cruise_name, vessel_name_abbr, zone, station, event_type, station_event_id, dfo_station_event_id, year, month, day, time_start, time_end,
...
# Join the individual event data frames from the 2022 IYS High Seas Expeditions and make the event_date column:
...
GoA2019_catch <- GoA2019_catch %>%
  ...
  select(Station, Station_Event_ID, Catch_ID, Species_recorded, Scientific_Name, Taxonomic_rank, species_code_russian, Lifestage, common_name, Catch_weight,
...
#TODO: Confirm this is the right approach and be consistent across other vessels
...
TINRO_CTD <- read_excel(here("input_datasets", "IYS2022_TINRO_Data.xlsx"), sheet = "7. CTD INFO") |> 
  rename(station_event_ID = Station_Event_ID,
         ...
         #TODO: confirm [chl] is for a 
         `sea_water_chl_a_concentration_ug/L` = sea_water_chl_concentration, 
         sea_water_temperature_degC = sea_water_temperature,
         sea_water_turbidity_NTU = sea_water_turbidity,
         sea_water_salinity_PSU = sea_water_salinity,
         `sea_water_EC25_uS/cm` = sea_water_EC25,
         `sea_water_dissolvedO2_sat_%` = sea_water_dissolvedO2_sat,
         `sea_water_density_kg/m3` = sea_water_density,
         `sea_water_conductivity_mS/cm` = sea_water_conductivity,
         sea_water_chl_fluorescence_PPB = sea_water_chl_fluorescence,
         `sea_water_BOD5_mL/L` = sea_water_BOD5, 
         `sea_water_dissolvedO2_mg/L`  = sea_water_dissolvedO2
         ) |> 
...
IYS_CTD <- bind_rows(TINRO_CTD, ctd_2019_data, ctd_2020) |> 
  select(station_event_ID:sampling_depth_dbar, sampling_depth_meters,
         ...
         `sea_water_concentration_of_oxygen_mL/L`, sea_water_pH:comments)
...
#TODO: Investigate Pacific Legacy sea water density units (range is off)
#TODO: Figure out why there isn't conductivity from TINRO vessel
#TODO: Figure out whether oxygen measurementments can be unit converted and combined

write_csv(IYS_CTD, here("output_datasets", "IYS2022_CTD.csv"))
...
# All IYS Expeditions:
out <- list("Data Dictionary" = data_dictionary, "Sampling Event Info" = IYS_events, "Overall_Catch_Data" = IYS_trawl_catch, "Overall_Specimen_Data" = IYS_trawl_specimen,
            "Overall_CTD_Data" = IYS2022_CTD)

# Only the 2022 Expeditions:
out_2022 <- list("Sampling Event Info" = IYS2022_events, "Overall_Catch_Data" = IYS2022_trawl_catch, "Overall_Specimen_Data" = IYS2022_trawl_specimen)

writexl::write_xlsx(out_2022, here("output_datasets", "master2022.xlsx"))
writexl::write_xlsx(out, here("output_datasets", "master.xlsx")
```
