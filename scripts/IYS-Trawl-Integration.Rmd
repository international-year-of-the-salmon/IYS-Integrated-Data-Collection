...
zoop_2019_2020 <- zoop_2019_2020 %>%
  mutate(net_number = "1",
         station_event_id = ifelse(grepl("2019", date), 
                                   paste("IYS2019", "Kaganovsky", station, "Bongo", sep = "-"),
                                   paste("IYS2020", "Legacy", station, "Bongo", sep = "-")),
         catch_id = paste(station_event_id, net_number, sep = "-")) %>%
...
zoop_2022 <- zoop_2022 %>%
  rename(specimen_id = catch_id, 
         catch_id = station_event_id) %>%
  mutate(station_event_id = sub('-[^-]*$', '', catch_id))
...
# Reshuffle columns:
zoop <- zoop %>% 
  dplyr::select(station_event_id, catch_id, specimen_id, scientific_name, verbatim_identification, 
...
zoop$lifestage <-  gsub("C15", "C1-C5", zoop$lifestage) #TODO: Confirm
zoop$lifestage <-  gsub("C56", "C5/C6", zoop$lifestage) #TODO: Confirm
...
zoop_col_order <- c("station_event_id", "net_number", "catch_id", "specimen_id", "verbatim_identification", 
