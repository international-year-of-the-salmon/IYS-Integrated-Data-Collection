...
TINRO_event <- read_excel(here("input_datasets", "IYS2022_TINRO_Data.xlsx"), sheet = "4. SAMPLING EVENT INFO") %>%
  filter(Event_Type %in% c("Trawl", "CTD"))
Shimada_event <- read_excel(here("input_datasets", "IYS2022_Shimada_Data.xlsx"), sheet = "4. SAMPLING EVENT INFO") %>%
  filter(Event_Type %in% c("Trawl", "CTD"))
...
# Save event data:
write_csv(IYS2022_trawl_event, here("output_datasets", "IYS2022_trawl_event.csv"))
```
