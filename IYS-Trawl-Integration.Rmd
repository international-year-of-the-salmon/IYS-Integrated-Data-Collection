...
TINRO_event <- TINRO_event %>% 
  mutate(eventDate = paste(date, Time_Start)) %>%
  mutate(my.lt = as.POSIXlt(TINRO_event$eventDate))
TINRO_event$my.lt$hour = TINRO_event$my.lt$hour-12

TINRO_event <- TINRO_event %>%
  mutate(fixed_date = eventDate - hrs)
```
