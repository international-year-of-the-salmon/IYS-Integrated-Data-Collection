...
NW_event <- read_excel(here("IYS2022_NW_Data.xlsx"), sheet = "Sampling_Event_Info") %>%
  filter(Event_Type == "CanTrawl")
TINRO_event <- read_excel(here("IYS2022_TINRO_Data.xlsx"), sheet = "4. SAMPLING EVENT INFO") %>%
  filter(Event_Type %in% c("Trawl", "CTD"))
Shimada_event <- read_excel(here("IYS2022_Shimada_Data.xlsx"), sheet = "4. SAMPLING EVENT INFO") %>%
  filter(Event_Type %in% c("Trawl", "CTD"))
GoA2019_event <- read_excel(here("JuvSalmon_2019-01_catch.v20.12_FINAL_SUBMITTED (2).xlsx"), 
...
# Additionally, you'll notice that there are three duplicate Station_Event_IDs. Likely - though needs confirmation - all the catch and specimen data comes from the trawl events that were ~60 minutes in length. Therefore, events that lasted 15 minutes will likely have to be filtered out. 
...
NW_catch <- read_excel(here("IYS2022_NW_Data.xlsx"), sheet = "Catch_Info")
TINRO_catch <- read_excel(here("IYS2022_TINRO_Data.xlsx"), sheet = "5. CATCH_FINAL INFO")
Shimada_catch <- read_excel(here("IYS2022_Shimada_Data.xlsx"), sheet = "5. CATCH_FINAL INFO")
GoA2019_catch <- read_excel(here("JuvSalmon_2019-01_catch.v20.12_FINAL_SUBMITTED (2).xlsx"),
                            sheet = "CATCH_FINAL")
```
...
NW_specimen <- read_excel(here("IYS2022_NW_Data.xlsx"), sheet = "Specimen_Info")
TINRO_specimen <- read_excel(here("IYS2022_TINRO_Data.xlsx"), sheet = "6. SPECIMEN INFO")
Shimada_specimen <- read_excel(here("IYS2022_Shimada_Data.xlsx"), sheet = "6. SPECIMEN INFO")
...
```{r overall CTD, eval = FALSE}
#TODO: read in corrected 2022 TINRO CTD dataset? not sure if needed
#TODO Include 2019 and 2020 CTD data
TINRO_CTD <- read_excel(here("IYS2022_TINRO_Data.xlsx"), sheet = "7. CTD INFO")


IYS2022_CTD <- bind_rows(TINRO_CTD)

write_csv(IYS2022_CTD, "IYS2022_CTD.csv")
```
